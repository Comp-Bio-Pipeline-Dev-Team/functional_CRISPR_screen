...
run_pca <- function(norm_df,
  ...
  count_pca_table <- count_pca_results$x %>% 
    as_tibble(rownames = sample_col, optional = TRUE) %>% 
...
```{r}
test_count_pca_results <- prcomp(t(sgRNAnorm_combCounts_df),
                              center = TRUE,
                              scale. = TRUE)

test_count_pca_table <- test_count_pca_results$x %>% 
  as.data.frame() %>% 
  rownames_to_column(var = 'sampleid')
     #as_tibble(rownames = 'sampleid') 
    # left_join(meta, by = 'sampleid')

test_count_pca_table
```
...
sgRNA_corr_matrix <- as.data.frame(sgRNA_corr_res$CorrMatrix)
